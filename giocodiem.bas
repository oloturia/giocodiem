   ...
   3 REM ***********************
  10 PRINT "Il gioco consiste nel prendere  i'*'.            Te sei' 'e lasci una scia.       Se cozzi nella tuascia o  toc=chi un masso( )morirai.         Quandoprendi un'*'la scia cam=bia colore e non e'piu'pericolo=sa perche'puoi cozzare solo con=tro il tuo colore.            L'altro gioco invece consiste dievitare le stelle(+)."
 ...
 156 IF xbloc=10 AND ybloc=10 THEN NEXT i
...
1059 GO TO 235
1060 PRINT AT 0,0;"game over, punti "+STR$ (score): BEEP 1,-20
...
2010 INK 7: BORDER 0: PAPER 0: CLS : DIM b(23,31)
2020 FOR n=1 TO 20
2030 LET xstella=INT (RND*22): LET ystella=INT (RND*30)
2035 IF xstella=10 AND ystella=10 THEN NEXT n
2040 PRINT AT xstella,ystella; INK INT (RND*7)+1;"+"
2050 LET b(xstella+1,ystella+1)=1
2060 NEXT n
...
2090 IF b(psx+1,psy+1)=1 THEN GO TO 2250
2100 LET a$=INKEY$
2120 IF a$="6" THEN LET dir=6
2130 IF a$="7" THEN LET dir=7
2150 PAUSE 3
...
2195 IF psy=0 THEN BEEP 0.2,-10
2196 IF psy=0 THEN BORDER 6
2197 IF psy=30 THEN BEEP 0.2,0
2198 IF psy=30 THEN BORDER 1
2200 LET score=score+1
2210 IF psy<>0 AND psy<>30 THEN GO TO 2076
2220 LET xstella=INT (RND*22): LET ystella=INT (RND*30)
2230 PRINT AT xstella,ystella; INK INT (RND*7)+1;"+": LET b(xstella+1,ystella+1)=1
2240 GO TO 2076
...
9001 IF b(xo+1,yo+1)=1 THEN GO TO 9000
9010 IF b(xo,yo)=1 THEN GO TO 9000
9020 PRINT AT xo,yo;"*"
9030 RETURN
